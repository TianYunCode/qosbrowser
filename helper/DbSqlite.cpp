...
void DbSqlite::connect(const QString &dbPath)
{
    ...
    if (!m_db.open())
    {
        throw STR("打开数据库失败：%1 %2").arg(dbPath, m_db.lastError().text());
    }
}
...
QSqlQuery DbSqlite::exec(const QString &sql)
{
    ...
    if (!query.exec(sql))
    {
        throw STR("执行sql失败：%1 %2").arg(sql, query.lastError().text());
    }
    return query;
}
...
QSqlQuery DbSqlite::exec(const QString &sql, const QVariantList &variantList)
{
    ...
    {
        throw STR("预编译sql失败：%1 %2").arg(sql, query.lastError().text());
    }
    ...
    if (!query.exec())
    {
        throw STR("执行sql bindvalue失败：%1 %2").arg(sql, query.lastError().text());
    }
    return query;
}
